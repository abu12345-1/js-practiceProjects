...
// const form = document.querySelector(".text-gen");
...
//     article.innerHTML = `<p>${text[randomIndex]}</p>`;
// });

// function displayMenuItems(text) {
//     article.innerHTML = text.map(item =>
//         `<p>${item}</p>`
//     ).join('');
// };
...
const generateInput = document.querySelector("#generate");
const form = document.querySelector(".text-gen");
const article = document.querySelector(".text");
const button = document.getElementById("btn");
...
form.addEventListener("submit", function (e) {
    ...
    const numOfParas = generateInput.value;

    if (numOfParas <= 0) {
        article.innerHTML = `<p>Please enter a valid number of paragraphs.</p>`;

    } else {
        const selectedParas = [];
        for (let i = 0; i < numOfParas; i++) {
            const randomIndex = Math.floor(Math.random() * text.length);
            selectedParas.push(`<p>${text[randomIndex]}</p>`);
        }
        article.innerHTML = selectedParas.join("");
    }
    return;
